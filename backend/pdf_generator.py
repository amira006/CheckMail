...
def _draw_page(canvas, doc, meta):
    ...
    canvas.drawRightString(W - M, fy, "\u2709  checkmail@gmail.com")


# ── Story builders ────────────────────────────────────────────────
def _meta_block(meta, s):
    return [
        Paragraph(meta.get("date", "—"), s['CM_MetaDate']),
        Paragraph(meta.get("username", "—"), s['CM_MetaUser']),
        Paragraph(meta.get("sender", "—"), s['CM_MetaLine']),
        Paragraph(meta.get("subject", "—"), s['CM_MetaLine']),
    ]
...
def _body_block(report, meta, s):
    ...
    username = meta.get("username", "Utilisateur")

    story = []

    # Salutation
    story.append(Paragraph(f"To {username},", s['CM_Salutation']))
    story.append(Spacer(1, 0.3 * cm))

    # Intro
    story.append(Paragraph(
        _build_intro(verdict, meta.get("sender"), meta.get("subject")),
        s['CM_Body']
    ))
...
def _signature_block(s):
    story = []
    story.append(Paragraph("Best regards,", s['CM_BestRegards']))
    story.append(Spacer(1, 0.25 * cm))

    # Signature image (optionnelle — pas d'erreur si absente)
    if os.path.isfile(SIG_PATH):
        try:
            story.append(RLImage(SIG_PATH, width=5.5 * cm, height=2.2 * cm))
        except Exception:
            pass
