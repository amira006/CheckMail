...
/* ── DATA ── */
const EMAILS = [
  { id:1, senderName:"Ahmed Benali", senderEmail:"ahmed.benali@gmail.com", subject:"Facture #2024-0512 — Paiement urgent requis", date:"2024-06-10", time:"09:14", size:"348 Ko", status:"Infecté", score:94, attachments:2, attachmentNames:["facture.pdf","paiement.exe"], headers:"Received: from mail.spam-domain.ru\nX-Mailer: PhishKit v3", bodyPreview:"Veuillez régler cette facture dans les 24h ou votre compte sera suspendu.", threats:[{icon:"",text:"Exécutable malveillant joint (paiement.exe)",level:"high"},{icon:"",text:"Lien de phishing détecté dans le corps",level:"high"},{icon:"",text:"Expéditeur usurpant un domaine légitime",level:"medium"}] },
  ...
  { id:5, senderName:"Amazon.fr", senderEmail:"confirm@amaz0n-delivery.info", subject:"Votre colis est en attente — Frais de douane", date:"2024-06-09", time:"11:17", size:"88 Ko", status:"Suspect", score:71, attachments:1, attachmentNames:["suivi_colis.html"], headers:"Received: from amaz0n-delivery.info\nSPF: softfail", bodyPreview:"Votre colis est bloqué en douane. Veuillez payer 2,99 € pour le libérer.", threats:[{icon:"",text:"Domaine suspect imitant Amazon",level:"medium"},{icon:"",text:"Fichier HTML joint avec scripts embarqués",level:"medium"},{icon:"",text:"Demande de paiement non sollicitée",level:"medium"}] },
...
function Modal({ email, onClose }) {
  ...
  const verdictMap = {
    ...
    suspect:  { icon:'', title:'Email Suspect',    desc:'Des éléments suspects ont été détectés. Prudence recommandée.' },
  ...
  return (
    <div className="hist-page">
      ...
      <div className="hist-toolbar">
        <input className="hist-search" placeholder="Rechercher un email…" value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}} />
            ...
            {slice.length === 0 ? (
              ...
              return (
                <tr key={e.id} style={{animationDelay:`${i*40}ms`}}>
                  ...
                  <td>
                    <button className="btn-view" onClick={()=>setModal(e)}>Voir</button>
                  </td>
                </tr>
              );
            })}
