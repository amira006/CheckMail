  ...
  return str.replace(/=\?([^?]+)\?([BbQq])\?([^?]*)\?=/g, (_, charset, encoding, encoded) => {
    try {
      if (encoding.toUpperCase() === 'B') {
        ...
        const qp = encoded.replace(/_/g, ' ').replace(/=([0-9A-Fa-f]{2})/g,
...
export default function Results({ emlData }) {
  ...
  // FIX 2 : décodage MIME + capture sujet multi-lignes
  const meta = emlData?.content ? {
    from:    decodeMimeHeader((emlData.content.match(/^From:\s*(.+)$/im) || [])[1] || '—'),
    subject: decodeMimeHeader(
      (emlData.content.match(/^Subject:\s*([\s\S]+?)(?=\r?\n\S|\r?\n\r?\n)/im) || [])[1]
        ?.replace(/\r?\n\s+/g, ' ').trim() || '—'
    ),
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      console.log('🔍 Réponse brute du serveur:', JSON.stringify(data).substring(0, 300));

      // ── Lecture du rapport : supporte ancien ET nouveau format ──────────────
      let rep = null;

      // Format nouveau : data.report est un objet direct
      if (data.report && typeof data.report === 'object' && data.report.score !== undefined) {
        rep = data.report;
        console.log('✅ Format nouveau détecté, score =', rep.score);
      }

      // Format ancien : JSON noyé dans data.result (fallback)
      if (!rep) {
        const text2 = data.result || '';
        const jm = text2.match(/\{[\s\S]*?"score"[\s\S]*?"verdict"[\s\S]*?\}/);
        if (jm) { try { rep = JSON.parse(jm[0]); console.log(' Format ancien détecté, score =', rep.score); } catch(e) {} }
      }

      if (rep) setReport(rep);
      else console.error(' Impossible de lire le rapport. data =', data);

      const text = data.result || '';
      let msg = text.replace(/\{[\s\S]*?"score"[\s\S]*?"verdict"[\s\S]*?\}/, '').trim();
      ...
      else addBot('Analyse terminée. Posez-moi vos questions sur cet email.', chips);
        ...
        <div>
          ...
          <table className="rp-table">
            <tbody>
              ...
              <tr><td>Expéditeur</td>     <td style={{wordBreak:'break-all'}}>{meta.from || '—'}</td></tr>
              {/* FIX 2 : word-break + white-space pour afficher l'objet en entier */}
              <tr><td>Objet</td>          <td style={{wordBreak:'break-word', whiteSpace:'normal'}}>{meta.subject || '—'}</td></tr>
